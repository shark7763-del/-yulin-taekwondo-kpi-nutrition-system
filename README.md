...
6. 分批確認選手 PIN 與家長手機登入正常後，再關閉「舊制過渡登入」。

安全規則：PIN 與教練密碼不儲存明碼；錯誤 5 次鎖定 10 分鐘；登入 session 最長 6 小時。家長首次完整手機驗證後，必須完成個資告知同意才能取得孩子摘要。
...
7. 關閉舊制過渡登入後，名單、全隊紀錄、家長資料與教練設定 API 都要求後端角色 session；不要重新開啟舊制做為長期登入方式。
